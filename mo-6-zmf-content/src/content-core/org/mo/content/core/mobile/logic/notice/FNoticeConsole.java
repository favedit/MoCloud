         ...
         INoticeConsole
{
   ...
   public FDataLogicNoticeUnit markRead(long userId,
      ...
      long resultLong = personUserNoticeLogic.connection().executeLong("SELECT MIN(`NOTICE_ID`) FROM `LG_PSN_USER_NOTICE` WHERE `USER_ID`=" + userId);
      _logger.debug(this, "markRead", "markRead From FNoticeConsole begin resultLong={1}", resultLong);
      // 如果是0意味着没有找到,把该用户的最新公告标志已读同时返回下一条最新公告
      FSql whereSqlFirst = new FSql();
      whereSqlFirst.append(FDataLogicNoticeLogic.UPDATE_DATE);
      whereSqlFirst.append(" =(SELECT MAX(`UPDATE_DATE`) FROM `DT_LGC_NOTICE` AS N1 WHERE `N1`.`STATUS_CD`=2 AND `N1`.`DISPLAY_CD`=1)");
      FLogicDataset<FDataLogicNoticeUnit> noticeUnitsFirst = noticeLogic.fetch(whereSqlFirst);
      if(resultLong == 0){
         if(noticeUnitsFirst != null && noticeUnitsFirst.count() > 0){
            ...
            whereSql2.append(" =(SELECT MAX(`UPDATE_DATE`) FROM `DT_LGC_NOTICE` AS N1 WHERE `N1`.`STATUS_CD`=2 AND `N1`.`DISPLAY_CD`=1 AND `N1`.`OUID`<" + noticeUnit.ouid() + ")");
         ...
         whereSql3.append(" =(SELECT MAX(`UPDATE_DATE`) FROM `DT_LGC_NOTICE` AS N1 WHERE `STATUS_CD`=2 AND `DISPLAY_CD`=1 AND `OUID`<" + resultLong + ")");
         FLogicDataset<FDataLogicNoticeUnit> noticeUnits = noticeLogic.fetch(whereSql3);
         if(noticeUnits != null && noticeUnits.count() > 0){
            ...
            whereSql4.append(" =(SELECT MAX(`UPDATE_DATE`) FROM `DT_LGC_NOTICE` AS N1 WHERE `N1`.`STATUS_CD`=2 AND `N1`.`DISPLAY_CD`=1 AND `N1`.`OUID`<" + noticeUnits.first().ouid() + ")");
            return noticeLogic.fetch(whereSql4).first();
         }else{
            // 1 √ 2 √ 3 × 4 ×逻辑
            FSql whereSql5 = new FSql();
            whereSql5.append(FDataPersonUserNoticeLogic.UPDATE_DATE);
            whereSql5.append("=(SELECT MIN(`UPDATE_DATE`) FROM `LG_PSN_USER_NOTICE` AS PUN1  WHERE PUN1.`USER_ID`=" + userId + ")");
            FDataPersonUserNoticeUnit minUnit1 = personUserNoticeLogic.fetch(whereSql5).first();

            FSql whereSql6 = new FSql();
            whereSql6.append(FDataPersonUserNoticeLogic.UPDATE_DATE);
            whereSql6.append("=(SELECT MAX(`UPDATE_DATE`) FROM `LG_PSN_USER_NOTICE` AS PUN1  WHERE PUN1.`USER_ID`=" + userId + ")");
            FDataPersonUserNoticeUnit maxUnit2 = personUserNoticeLogic.fetch(whereSql6).first();

            FSql whereSql7 = new FSql();
            whereSql7.append(FDataPersonUserNoticeLogic.UPDATE_DATE);
            whereSql7.append("=(SELECT MAX(`UPDATE_DATE`) FROM `DT_LGC_NOTICE` AS N1 WHERE `N1`.`STATUS_CD`=2 AND `N1`.`DISPLAY_CD`=1 AND N1.OUID<" + maxUnit2.noticeId() + " AND N1.OUID>" + minUnit1.noticeId() + ")");
            FLogicDataset<FDataLogicNoticeUnit> noticeUnits2 = noticeLogic.fetch(whereSql7);
            ...
            whereSql10.append(FDataPersonUserNoticeLogic.NOTICE_ID);
            whereSql10.append("=");
            whereSql10.append(noticeUnitFirstOuid);
            FLogicDataset<FDataPersonUserNoticeUnit> fetch2 = personUserNoticeLogic.fetch(whereSql10);
            if(fetch2 == null || fetch2.count() < 1){
               ...
               whereSql9.append("=(SELECT MAX(`UPDATE_DATE`) FROM `DT_LGC_NOTICE` AS N1 WHERE `N1`.`STATUS_CD`=2 AND `N1`.`DISPLAY_CD`=1 AND N1.OUID<" + noticeUnitFirstOuid + ")");
               FLogicDataset<FDataLogicNoticeUnit> fetchSS = noticeLogic.fetch(whereSql9);
            ...
            if(noticeUnits2 != null && noticeUnits2.count() > 0){
               ...
               whereSql8.append("=(SELECT MAX(`UPDATE_DATE`) FROM `DT_LGC_NOTICE` AS N1 WHERE `N1`.`STATUS_CD`=2 AND `N1`.`DISPLAY_CD`=1 AND N1.OUID<" + noticeUnits2.first().ouid() + " AND N1.OUID>" + minUnit1.noticeId() + ")");
               FLogicDataset<FDataLogicNoticeUnit> fetch = noticeLogic.fetch(whereSql8);
