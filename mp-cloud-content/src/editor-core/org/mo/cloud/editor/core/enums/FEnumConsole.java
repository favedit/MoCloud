...
import org.mo.cloud.editor.common.source.REditorCsSource;
import org.mo.cloud.editor.core.enums.common.XEnum;
import org.mo.cloud.editor.core.enums.common.XEnumGroup;
import org.mo.cloud.editor.core.enums.common.XEnumItem;
         ...
         IEnumConsole
{
   ...
   @AProperty
   protected String _pathPlatformAndroid;

   @AProperty
   protected String _pathPlatformIos;
   ...
   protected void buildAllJava(){
      ...
      for(XEnumGroup xgroup : list()){
         ...
         FXmlNode config = buildConfig(xgroup, EXmlConfig.Simple);
         String groupName = config.get("name");
         // 生成集合代码
         for(FXmlNode xenum : config.nodes()){
            if(!RBoolean.parse(xenum.get(XEnum.PTY_IS_CLIENT_JAVA, null))){
               continue;
            }
            xenum.set("group_name", groupName);
            String name = xenum.get("name");
            //............................................................
            // 建立Java枚举代码
            String path = RFile.makeFilename(sourcePlatformPath(), groupName + "/E" + name + ".java");
            FString source = sourceBuilder.makeJavaSource(xenum);
            RFile.saveToFile(path, source, REncoding.UTF8);
            _logger.debug(this, "buildAllJava", "Build java enum source. (file={1})", path);
            // 建立Java集合代码
            if(RBoolean.parse(xenum.get(XEnum.PTY_IS_SET, null))){
               path = RFile.makeFilename(sourcePlatformPath(), groupName + "/T" + name + "Set.java");
               source = sourceBuilder.makeJavaSetSource(xenum);
               RFile.saveToFile(path, source, REncoding.UTF8);
               _logger.debug(this, "buildAllJava", "Build java set source. (file={1})", path);
            }
            //............................................................
            // 建立Android枚举代码
            path = RFile.makeFilename(sourcePlatformAndroidPath(), groupName + "/E" + name + ".java");
            source = sourceBuilder.makeJavaAndroidSource(xenum);
            RFile.saveToFile(path, source, REncoding.UTF8);
            _logger.debug(this, "buildAllJava", "Build java enum source. (file={1})", path);
            // 建立Android集合代码
            if(RBoolean.parse(xenum.get(XEnum.PTY_IS_SET, null))){
         ...
         //............................................................
         // 建立IOS枚举代码
         String path = RFile.makeFilename(sourcePlatformIosPath(), "Model" + RString.firstUpper(groupName) + "Enum.h");
         FString source = sourceBuilder.makeIosSource(config);
         RFile.saveToFile(path, source, REncoding.UTF8);
         _logger.debug(this, "buildAllIos", "Build java ios source. (file={1})", path);
      }
   }
   ...
   //============================================================
   public String sourcePlatformAndroidPath(){
      return RFile.format(_pathPlatformAndroid);
   }

   //============================================================
   public String sourcePlatformIosPath(){
      return RFile.format(_pathPlatformIos);
   }
