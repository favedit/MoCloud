package org.mo.cloud.editor.core.dataset;

import org.mo.cloud.editor.core.dataset.base.XBaseDataStore;
import org.mo.cloud.editor.core.dataset.common.FDatasetSourceBuilder;
import org.mo.cloud.editor.core.dataset.common.RDatasetSqlBuilder;
import org.mo.cloud.editor.core.dataset.common.XDataStore;
         ...
         IDatasetConsole
{
   ...
   //============================================================
   @Override
   public void buildAll(EDatasetSourceType sourceCd){
      if((EDatasetSourceType.All == sourceCd) || (EDatasetSourceType.JavaAll == sourceCd)){
         buildAllJava();
      }
   }
   ...
   public void build(FDatasetBuilderArgs builderArgs){
      ...
      if(EDatasetSourceType.All == type || EDatasetSourceType.JavaAll == type){
         String groupName = RString.left(dataName, ".");
         String groupPath = _sourceJavaDataPath + groupName;
         String packageName = "org.mo.cloud.content.data.logic." + groupName.toLowerCase();
         FXmlNode config = buildConfig(dataName);
   ...
   public void buildAllJava(){
      // 获取传过来的类型（FXmlNode,XDataset类型）
      FDatasetSourceBuilder sourceBuilder = new FDatasetSourceBuilder(_templateConsole);
      // 获得设置信息
      for(IXmlObject xdataset : list()){
         // 建立XML节点信息
         FXmlNode dataset = convertInstanceToNode(xdataset, EXmlConfig.Simple);
         if(dataset.isName("DataTemplate")){
            continue;
         }
         // 建立模板控制台
         String name = RString.replace(xdataset.innerGet(XDataStore.PTY_NAME), '.', '_');
      ...
      try{
         ...
         for(FXmlNode fieldNode : fieldsNode){
            ...
            if(RString.isNotEmpty(dataRefer)){
               ...
               fieldNode.set("refer_field_name", RString.left(fieldNode.get("name"), "_id"));
               fieldNode.set("refer_class_name", "F" + RDatabaseFormat.toJavaClassName(dataRefer.replace('.', '_')));
            }
            // 设置数据集内容
            ...
            if("RecordId".equals(fieldType)){
               ...
               fieldType = "DOUBLE";
               fieldNode.set("java_type", "double");
            ...
            if(RString.isNotEmpty(dataUniques)){
               ...
               for(String unique : RString.split(dataUniques, ',')){
                  if(RString.isNotBlank(unique)){
                     ...
                     String value = uniqueNames.find(unique);
