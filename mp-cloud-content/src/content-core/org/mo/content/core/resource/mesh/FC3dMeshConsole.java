...
import org.mo.cloud.logic.resource.mesh.FGcResMeshConsole;
...
public class FC3dMeshConsole
      extends FGcResMeshConsole
      implements
         IC3dMeshConsole
{
   // 网格控制台接口
   @ALink
   protected IGcResMeshConsole _meshConsole;
   ...
   public FLogicDataset<FLogicUnit> list(ILogicContext logicContext,
                                         ...
                                         int pageSize,
                                         int page){
      FLogicDataset<FGcResMeshInfo> meshes = fetch(logicContext, whereSql, orderSql, pageSize, page);
      // 转换结果
   ...
   //============================================================
   @Override
   public FGcResMeshInfo createMesh(ILogicContext logicContext,
                                    FGcResMeshInfo mesh){
      // 创建资源对象
   ...
   public EResult doDeleteByGuid(ILogicContext logicContext,
                                 long userId,
                                 String guid){
      FGcResMeshInfo mesh = findByGuid(logicContext, guid);
      // 网格存在检查
      if(mesh == null){
   ...
   public EResult doDeleteByResourceId(ILogicContext logicContext,
                                       long userId,
                                       long resourceId){
      FGcResMeshInfo mesh = findByResourceId(logicContext, resourceId);
      // 网格存在检查
      if(mesh == null){
