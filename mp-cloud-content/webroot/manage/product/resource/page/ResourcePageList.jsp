...
$(function(){	
	doSubmit(null);
	var pager =	$('#resourcePage').datagrid().datagrid('getPager');
	pager.pagination({
			pageSize: 20,
			showPageList:false,
...
function doSubmit(page){
	...
	var data = null;
	if(page != null){
		url = "/cloud/product/resource/page/ResourcePage.wa?do=selectByPage&page="+page+"&date="+new Date().valueOf();
		data ={"label":$('#label').val(),"page":page};
	}else{
		url = "/cloud/product/resource/page/ResourcePage.wa?do=selectByPage&date="+new Date().valueOf();
	}
	$.ajax({
	   type: "POST",
	   url:url,
	   ...
	   success: function(msg){
		 closeProgress();
		 $('#resourcePage').datagrid('loadData', toJsonObject(msg));
	   },
	   fail:function(){
		 closeProgress();
	   	 alert("error");
...
function del(ouid){
	...
	function(){
		var url = "/cloud/product/resource/page/ResourcePage.wa?do=delete&date="+new Date().valueOf();
		var data ={"ouid":ouid};
		$.getJSON(url,data,function(d){ 
			if(d=="1"){
				location.href="/cloud/product/resource/page/ResourcePage.wa";			
			}else{
...
function edit(id){
	location.href = "/cloud/product/resource/page/ResourcePage.wa?do=updateUpload&ouid="+id+"&date="+new Date().valueOf();
}

function jumpToInsertPage(){
	location.href="/cloud/product/resource/page/ResourcePage.wa?do=insertOnload";
}
...
<div id="cy_right" style="width:100%">
    ...
    <div class="btn_bar">
        <div class="nav_btn">
        <a href="/cloud/product/resource/page/ResourcePage.wa?do=insertOnload" class="add_btn"></a>
        <a id="regenerateIcon" hidden="true" href="/cloud/product/resource/page/ResourcePage.wa?do=regenerateIcon">重新生成图标</a>
...
<table id="resourcePage" class="easyui-datagrid" fit='true'
