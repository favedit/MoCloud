...
$(function(){	
	doSubmit(null);
	var pager =	$('#resourceModule').datagrid().datagrid('getPager');
	pager.pagination({
			pageSize: 20,
			showPageList:false,
...
function doSubmit(page){
	...
	var data = null;
	if(page != null){
		url = "/cloud/product/resource/module/ResourceModule.wa?do=selectByPage&page="+page+"&date="+new Date().valueOf();
		data ={"label":$('#label').val(),"page":page};
	}else{
		url = "/cloud/product/resource/module/ResourceModule.wa?do=selectByPage&date="+new Date().valueOf();
	}
	$.ajax({
	   type: "POST",
	   url:url,
	   ...
	   success: function(msg){
		 closeProgress();
		 $('#resourceModule').datagrid('loadData', toJsonObject(msg));
	   },
	   fail:function(){
		 closeProgress();
	   	 alert("error");
...
function del(ouid){
	...
	function(){
		var url = "/cloud/product/resource/module/ResourceModule.wa?do=delete&date="+new Date().valueOf();
		var data ={"ouid":ouid};
		$.getJSON(url,data,function(d){ 
			if(d=="1"){
				location.href="/cloud/product/resource/module/ResourceModule.wa";			
			}else{
...
function edit(id){
	location.href = "/cloud/product/resource/module/ResourceModule.wa?do=updateUpload&ouid="+id+"&date="+new Date().valueOf();
}

function jumpToInsertPage(){
	location.href="/cloud/product/resource/module/ResourceModule.wa?do=insertOnload";
}
...
<div id="cy_right" style="width:100%">
    ...
    <div class="btn_bar">
        <div class="nav_btn">
        <a href="/cloud/product/resource/module/ResourceModule.wa?do=insertOnload" class="add_btn"></a>
        <a id="regenerateIcon" hidden="true" href="/cloud/product/resource/module/ResourceModule.wa?do=regenerateIcon">重新生成图标</a>
...
<table id="resourceModule" class="easyui-datagrid" fit='true'
...
data-options="toolbar:'#cy_right',pagination:true,rownumbers:true,collapsible:true,singleSelect:true">
	<thead>
			<tr>
                  ...
                  <th data-options="field:'displayCd',halign:'center',align:'center',formatter:changeCheckbox" width="60px">显示</th>
