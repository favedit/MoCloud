...
// <T>上传处理。</T>
//============================================================
public class FUploadDirectServlet
      extends FObject
      implements
         IUploadDirectServlet
{
   // 日志输出接口
   private static ILogger _logger = RLogger.find(FUploadDirectServlet.class);
   ...
   public void process(IWebContext context,
      ...
      // 获得位图集合
      SGcStorageImages images = null;
      String imagesPack = context.parameter("images");
      if(!RString.isEmpty(imagesPack)){
         images = new SGcStorageImages();
      ...
      // 写入文件
      File storageFile = _storageConsole.createFile(catalog, code, code, name);
      _logger.debug(this, "process", "Upload data. (catalog={1}, type={2}, code={3}, size={4}, block_size={5}, file_name={6})", catalog, type, code, size, blockSize, storageFile.getAbsolutePath());
      try(RandomAccessFile file = new RandomAccessFile(storageFile, "rw")){
         ...
         // 建立图片集合
         if(images != null){
            //String storageFileName = storageFile.getAbsolutePath();
            //_pictureConsole.buildImages(catalog, storageFileName, images);
         }
      }catch(IOException e){
