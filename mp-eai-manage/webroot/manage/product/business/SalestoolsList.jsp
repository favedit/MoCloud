...
<script>
    ...
    function doSubmit(page, pageSize) {
        progress();
        var url = "/manage/product/business/Salestools.wa?do=select&date="
                + new Date().valueOf();
        var data = null;
        var label = $.trim($('#label').val()).replaceAll("'", "");
        if (label == "标题") label = null;
        if (page != null) {
            url = "/manage/product/business/Salestools.wa?do=select&page="
    ...
    function del(id) {
    	 return confirmx('确定删除?',
                 function() {
                     location.href = "/manage/product/business/Salestools.wa?do=delete&id=" + id + "&date=" + new Date().valueOf();
                 });
    }
    //更新配置信息-AnjoyTian
    function edit(id) {
        console.info(id);
        window.location.href = "/manage/product/business/Salestools.wa?do=updateBefore&id="
...
<body>
 <div id="cy_right" style="width: 100%">
  ...
  <div class="btn_bar">
   <div class="nav_btn">
    <a href="/manage/product/business/Salestools.wa?do=insertBefore"
     class="add_btn"></a>
   </div>
