...
<script>
   ...
   function doSubmit(page, pageSize) {
      progress();
      var url = "/manage/product/examine/business/notice/Notice.wa?do=select&date="
              + new Date().valueOf();
      var data = null;
      var label = $.trim($('#label').val()).replaceAll("'", "");
      if (label == "公告标题") label = null;
      if (page != null) {
          url = "/manage/product/examine/business/notice/Notice.wa?do=select&page="
   ...
   function phoneInfo(ouid){
      $('#w').window({
          ...
          height: 600,
          modal: true,
          href: "/manage/product/examine/business/notice/Notice.wa?do=getDescription&ouid="+ouid,
          title: "用户公告内容"
      });
   }
</script>
     ...
     <div class="nav_btn">
      ...
      <div class="nav_search">
         <input id="label" name="label" type="text"
           onfocus="if(this.value=='公告标题'){this.value='';}this.style.color='#000000';"
           onblur="if(this.value=='') {this.value='公告标题';this.style.color='#ccc';}"
           style="color: #ccc" value="公告标题"> <a onClick="doSubmit(0)"
