...
<script>
    ...
    function doSubmit(page, pageSize) {
        progress();
        var url = "/manage/product/examine/version/Version.wa?do=select&date="
                + new Date().valueOf();
        var data = null;
        var label = $.trim($('#label').val()).replaceAll("'", "");
        if (label == "资讯标题") label = null;
        if (page != null) {
            url = "/manage/product/examine/version/Version.wa?do=select&page="
...
-->
 ...
 <table id="logicVersion" class="easyui-datagrid" fit='true'
   style="align: true"
