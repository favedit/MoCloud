...
-- 修改客户金额的类型
ALTER TABLE `EAI_DATA`.`DT_FIN_CUSTOMER` CHANGE `REDEMPTION_TOTAL` `REDEMPTION_TOTAL` DOUBLE NULL, CHANGE `NETINVESTMENT` `NETINVESTMENT` DOUBLE NULL, CHANGE `INTEREST_TOTAL` `INTEREST_TOTAL` DOUBLE NULL; 

-- 151109 修改统计表 maocy
ALTER TABLE `ST_FIN_CUSTOMER` ADD COLUMN `INVESTMENT_LEVEL_CD`    INTEGER AFTER `INVESTMENT_TOTAL`; 
ALTER TABLE `ST_FIN_CUSTOMER` ADD COLUMN `REDEMPTION_LEVEL_CD`    INTEGER AFTER `REDEMPTION_TOTAL`; 
ALTER TABLE `ST_FIN_CUSTOMER` ADD COLUMN `NETINVESTMENT_LEVEL_CD` INTEGER AFTER `NETINVESTMENT_TOTAL`; 
