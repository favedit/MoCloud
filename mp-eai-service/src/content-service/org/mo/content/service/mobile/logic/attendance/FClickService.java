...
// <T>打卡服务。</T>
//============================================================
public class FClickService extends FObject implements IClickService {
    // 日志输出接口
    ...
    // ============================================================
    @Override
    public EResult sign(IWebContext context, IWebSession sessionContext,
            IWebInput input, IWebOutput output, ILogicContext logicContext) {
        FXmlNode inputNode = input.config();
        FXmlNode locationLongitudeNode = inputNode
                .findNode("locationlongitude");
        ...
        _logger.debug(this, "sign",
                "userguid={1},locationlongitude={3},locationlatitude={4}",
                sessionCode, locationLongitude, locationLatitude);
        // 所有的参数不能为空
        if ("".equals(sessionCode) || "".equals(locationLongitude)
                || "".equals(locationLatitude)) {
        ...
        FGcSessionInfo sessionInfo = _gcSessionConsole.findBySessionCode(
                logicContext, "eai.mobile", "0", sessionCode);
        long userId = -1;
        if (sessionInfo != null) {
            ...
            _clickConsole
                    .click(context, logicContext, sessionContext, singUser);
