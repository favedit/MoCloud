...
// <T>新闻服务。</T>
//============================================================
public class FNewsService extends FObject implements INewsService {
    // 日志输出接口
    ...
    public EResult query(IWebContext context, IWebInput input,
        ...
        FDataLogicNewsUnit newsUnit = _newsConsole.getNewsByGuid(guid,
                logicContext);
        if (newsUnit != null) {
    ...
    public EResult select(IWebContext context, IWebInput input,
        ...
        FLogicDataset<FDataLogicNewsUnit> newsUnits = _newsConsole.select(
                pageNum, pageSize, logicContext);
        ...
        if (newsUnits != null && newsUnits.count() > 0) {
            for (Iterator<FDataLogicNewsUnit> iterator = newsUnits.iterator(); iterator
                    .hasNext();) {
                ...
                if (guidStr != null && (!"".equals(guidStr))) {
                    ...
                    if (newsUnit.linkCd() == EGcLink.Content) {
                        xruntime.createNode("info_url")
                                .setText(
                                        _newsServiceHost
                                                + "mobile/logic/news/News.wa?do=getNewInfo&guid="
                                                + newsUnit.guid());
                    }
                } else {
