...
// <T>新闻服务。</T>
//============================================================
public class FSalesToolsService extends FObject implements ISalesToolsService {
    // 日志输出接口
    ...
    public EResult query(IWebContext context, IWebInput input,
        ...
        FDataLogicSalestoolsUnit newsUnit = _salesToolsConsole.getNewsByGuid(
                guid, logicContext);
        if (newsUnit != null) {
    ...
    public EResult select(IWebContext context, IWebInput input,
            IWebOutput output, ILogicContext logicContext) {
        _logger.debug(this, "FSalesToolsService_select",
                "FSalesToolsService_select begin. ");
        ...
        FLogicDataset<FDataLogicSalestoolsUnit> salesToolsUnits = _salesToolsConsole
                .select(pageNum, pageSize, logicContext);
        ...
        if (salesToolsUnits != null && salesToolsUnits.count() > 0) {
            ...
            for (Iterator<FDataLogicSalestoolsUnit> iterator = salesToolsUnits
                ...
                if (salesToolsUnit.guid() != null
                        && (!"".equals(salesToolsUnit.guid()))) {
                    xruntime.createNode("guid").setText(salesToolsUnit.guid());
                    if (salesToolsUnit.linkCd() == EGcLink.Content) {
                        xruntime.createNode("info_url")
                                .setText(
                                        _salesServiceHost
                                                + "mobile/logic/salestools/SalesTools.wa?do=getContent&guid="
                                                + salesToolsUnit.guid());
                    }
                } else {
                ...
                if (salesToolsUnit.linkCd() == EGcLink.Link) {
                    if (salesToolsUnit.linkUrl() != null
                            && (!"".equals(salesToolsUnit.linkUrl()))) {
                        xruntime.createNode("info_url").setText(
                                salesToolsUnit.linkUrl());
                    } else {
                        xruntime.createNode("info_url").setText("0");
                    }
                }

                if (salesToolsUnit.label() != null
                        && (!"".equals(salesToolsUnit.label()))) {
                    xruntime.createNode("label")
                            .setText(salesToolsUnit.label());
                } else {
                    xruntime.createNode("label").setText("0");
                }
                if (salesToolsUnit.description() != null
                        && (!"".equals(salesToolsUnit.description()))) {
                    xruntime.createNode("description").setText(
                            salesToolsUnit.description());
                } else {
