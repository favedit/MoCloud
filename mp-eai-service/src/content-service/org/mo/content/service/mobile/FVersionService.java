...
// <T>版本更新服务。</T>
//============================================================
public class FVersionService extends FObject implements IVersionService {
    // 日志输出接口
    ...
    // ============================================================
    @Override
    public EResult connect(IWebContext context, IWebSession sessionContext,
        ...
        /*
         * FXmlNode sessionNode = inputNode.findNode("mo-session-id"); String
        ...
        _logger.debug(
                this,
                "****************************------------------------------------->show parameters",
                "appos={1},sessionCode={2}", applicationStr, sessionStr);
        String versionStr = "";
        System.out.println("****************************---------------->"
                + sessionContext.getClass().getName());
        // 输出信息
        HashMap<String, Object> hashMap = _versionConsole.connect(context,
                versionStr, applicationStr, logicContext, sessionContext);
        FDataSystemVersionUnit lastVersionUnit = (FDataSystemVersionUnit) hashMap
                .get("lastVersion");
        ...
        if (lastVersionUnit != null && lastVersionUnit.number() > numberFloat) {
            ...
            String code = lastVersionUnit.label();
            ...
            // outputVersionNode.set("upgrade_size",
            // lastVersionUnit.downloadSize());
        } else {
        ...
        if ("".equals(sessionStr)) {
            ...
            // 如果有值,验证session是否有效
            FGcSessionInfo sessionInfo = _gcSessionConsole.findBySessionCode(
                    logicContext, "eai.mobile", "0", sessionStr);
            // 如果session无效
            if (sessionInfo == null) {
                FXmlNode sessionCode = output.config().createNode(
                        "session_code");
                FGcWebSession session = (FGcWebSession) sessionContext;
                sessionCode.setText(session.id());
            } else {
    ...
    // ============================================================
    @Override
    public EResult disconnect(IWebContext context, IWebSession sessionContext,
            IWebInput input, IWebOutput output, ILogicContext logicContext) {
        // 获取会话id
        FXmlNode inputNode = input.config();
        FXmlNode inputSessionCodeNode = inputNode.findNode("SessionCode");
        String sessionCodeNodeStr = inputSessionCodeNode.text();
        _versionConsole.disconnect(context, sessionCodeNodeStr, logicContext,
                sessionContext, _sessionConsole);
        return EResult.Success;
    }
}
