...
// <T>登录控制台。</T>
//============================================================
public class FPersonInfoConsole extends FObject implements IPersonInfoConsole {
    // 日志输出接口
    ...
    public FDataPersonUserUnit login(IWebContext context, String passport,
        ...
        _logger.info(this, "login", "User login. (passport={1}, message={1})",
                passport, logggerMessage);
        // 如果登录成功,同步数据
        ...
        // 增加日志
        FLoggerPersonUserAccess logger = _loggerPersonUserAccessConsole
                .doPrepare(logicContext);
        logger.setUserId(1);
    ...
    public String oaLogin(String passport, String password) {
        ...
        String from = "H5";
        String validate = RMd5.encode(encodePassport + encodePassword + appDate
                + from + key);
        // 拼装参数
        String parem = null;
        try {
            parem = "?username=" + URLEncoder.encode(encodePassport, "utf-8")
                    + "&pwd=" + URLEncoder.encode(encodePassword, "utf-8")
                    + "&appDate=" + appDate + "&from=" + from + "&validate="
                    + validate;
        } catch (Exception error) {
            _logger.error(null, "oaLogin", error, "ROALoginUnit oaLogin",
                    "OA login fail.");
        }
        _logger.debug(null, "ROALoginUnit oaLogin", "OA login. (url={1})",
                _oaLoginUrl + parem);
        // 发送请求
        String result = null;
        try (FHttpConnection connection = new FHttpConnection(_oaLoginUrl
                + parem)) {
    ...
    public FDataPersonUserUnit synchronizeData(ILogicContext logicContext,
        ...
        sql.append("'");
        FDataPersonUserLogic userLogic = logicContext
                .findLogic(FDataPersonUserLogic.class);
        FLogicDataset<FDataPersonUserUnit> findUserByGuids = userLogic
                .fetch(sql);

        FDataPersonUserEntryLogic userEntryLogic = logicContext
                .findLogic(FDataPersonUserEntryLogic.class);
        ...
        if (findUserByGuids == null || findUserByGuids.count() < 1) {
            ...
            FLogicDataset<FDataPersonUserEntryUnit> updateUserEntryUnits = userEntryLogic
                    .fetch(whereSql);
            if (updateUserEntryUnits != null
                    && updateUserEntryUnits.count() > 0) {
    ...
    public FDataPersonInfo getUserInfo(long user_id,
        ...
        FDataPersonUserSigningLogic signingLogic = logicContext
                .findLogic(FDataPersonUserSigningLogic.class);
        FDataPersonUserLogic userLogic = logicContext
                .findLogic(FDataPersonUserLogic.class);
        ...
        FLogicDataset<FDataPersonInfo> userInfos = userLogic.fetchClass(
                FDataPersonInfo.class, userFSql);
        if (userInfos != null && userInfos.count() > 0) {
            userInfo = userInfos.first();
        }

        // %Y-%m-%d %T
        String sql = "SELECT DATE_FORMAT(MAX(SINGN_DATE) ,'%Y-%m-%d %H:%i:%s') AS LAST_SIGN_DATE FROM DT_PSN_USER_SIGNING WHERE `USER_ID`="
                + user_id;
        FRow rowData = signingLogic.connection().find(sql);
        String Last_sign_date = rowData.get("last_sign_date");
        _logger.debug(this, "getUserInfo",
                "----------------------------*****>Last_sign_date={1}",
                Last_sign_date);
    ...
    public FDataSystemApplicationUnit getApplicationUnitByGuid(
        ...
        FDataSystemApplicationLogic applicationLogic = logicContext
                .findLogic(FDataSystemApplicationLogic.class);
    ...
    public void clearSessionByUserId(long userId, long appId,
        ...
        FLogicDataset<FGcSessionInfo> sessionInfos = _sessionConsole
                .findByUserAndAppId(logicContext, userId, appId);
        for (Iterator<FGcSessionInfo> iterator = sessionInfos.iterator(); iterator
                .hasNext();) {
