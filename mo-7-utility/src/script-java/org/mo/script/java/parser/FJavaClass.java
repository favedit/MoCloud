...
import org.mo.com.collections.FParameters;
      ...
      extends FJavaObject
{
   ...
   // @return 类对象
   //============================================================
   public FJavaClass findClass(FJavaContent content,
                               String className){
      // 获取类名称
   ...
   // @return 类对象
   //============================================================
   public boolean testOverride(FJavaContent content,
                               String methodName){
   ...
   // @param source 代码
   //============================================================
   public String makeTypeName(FJavaParserContent content,
                              String name){
      // 获取类名称
   ...
   // <T>解析文件。</T>
   //============================================================
   public void parseChild(FJavaParserContent content,
                          FTextToken token){
      ...
      for(int lineNumber = 0; lineNumber < lineCount; lineNumber++){
         ...
         if(line.startsWith("@APublish")){
            isPublish = true;
            String pack = RString.mid(line, "(", ")");
            if(!RString.isEmpty(pack)){
               FParameters parameters = new FParameters();
               parameters.parse(pack);
               publishVersion = parameters.get("version");
            }
            publishVersion = line = lines[++lineNumber];
         }
         // 获得代码
         ...
         function.setPublishVersion(publishVersion);
         function.setIsGetter(false);
         function.setIsSetter(false);
         function.comment().assign(comment);
         function.parse(content, token, source);
         ...
         if(line.startsWith("package ")){
            ...
            if(packageToken.hasToken()){
               ...
               for(int n = 0; n < count; n++){
                  ...
                  if(_isInterface){
                     childToken.splite(';');
                     for(FTextToken spliteToken : childToken.tokens()){
                        parseChild(content, spliteToken);
                     }
                  }else{
   ...
   public void convert(FJavaConvertContent content,
      ...
      for(int n = 0; n < functionCount; n++){
         FJavaFunction function = _functions.get(n);
         source.appendLine();
   ...
   public void saveConfig(FXmlNode xconfig){
      ...
      // 保存属性
      xconfig.set("parent_class_name", _parentClassName);
      // 保存函数集合
      if(_functions != null){
         FXmlNode xfunctions = xconfig.createNode("Functions");
         for(FJavaFunction function : _functions){
            function.saveConfig(xfunctions.createNode("Function"));
         }
      }
   }
