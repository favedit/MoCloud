...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/design/list/ContentList.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='列表定义' icon='sys.pst.mgr' target='frmMain' page='#/design/list/ContentList.wa?do=list&amp;form_name=content.list.ListList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='content.design.list'/>
<!-- Body begin ---------------------------------------------->
