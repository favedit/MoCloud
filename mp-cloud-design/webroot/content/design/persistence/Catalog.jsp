...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/design/persistence/ContentPersistence.wa?do=update';
	fmMain.submit();
}
//----------------------------------------------------------
function _onloadAll(){
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnPersistence' label='持久化对象列表' icon='sys.pst.mgr' target='frmMain' page='#/design/persistence/ContentPersistence.wa?do=list&amp;form_name=content.persistence.PersistenceList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='content.design.persistence'/>
<!-- Body begin ---------------------------------------------->
