...
function onNodeClick(sender, node){
	...
	if('collection' == type){
		fmMain.sel_collection.value = node.get('linker_name');
		fmMain.sel_component.value = '';
	}else if('component' == type){
      var colNode = node.topNodeByType('collection');
		fmMain.sel_collection.value = colNode.get('linker_name');
		fmMain.sel_component.value = node.uuid;
	}else{
		return;
	}
   fmMain.action = '<jh:context/>/design/frame/ContentFrame.wa?do=update';
	fmMain.submit();
}
//----------------------------------------------------------
...
function onSearch(){
   ...
   if(s){
      ...
      tree.service = 'search@content.design.frame';
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='表单定义' icon='sys.pst.mgr' target='frmMain' page='#/design/frame/ContentFrame.wa?do=list&amp;form_name=content.frame.FrameList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='content.design.frame'/>
<!-- Body begin ---------------------------------------------->
