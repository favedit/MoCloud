...
public class FSynchronizeStatisticsMemberData
{
   ...
   public static void synchronizedMember(FLogicContext logicContext){
      ...
      //FStatisticsFinancialDynamicLogic statisticsDynamicLogic = logicContext.findLogic(FStatisticsFinancialDynamicLogic.class);
   ...
   public static void loadSTDynamicToDTCustomer(FDataFinancialCustomerUnit dataCustomerUnit,
      ...
      whereCustomerId.append(FStatisticsFinancialDynamicLogic.CUSTOMER_ACTION_DATE);
      whereCustomerId.append("= (SELECT MAX(`CUSTOMER_ACTION_DATE`) FROM `ST_FIN_DYNAMIC` AS D1 WHERE D1.`CUSTOMER_ID`=" + firstStatisticsFinancialCustomerUnit.ouid() + ")");
      FLogicDataset<FStatisticsFinancialDynamicUnit> statisticsDynamicUnits = statisticsDynamicLogic.fetch(whereCustomerId);
      if(statisticsDynamicUnits != null && statisticsDynamicUnits.count() > 0){
         FStatisticsFinancialDynamicUnit statisticsDynamicUnit = statisticsDynamicUnits.first();
         dataCustomerUnit.setMarketerId(statisticsDynamicUnit.marketerId());
         dataCustomerUnit.setMarketerLid(statisticsDynamicUnit.marketerLinkId());
      }
   }
