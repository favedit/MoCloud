...
public class FLoggerInfoBrowserAccessLogic extends FLogicTable
{
   ...
   // 信息浏览器访问的定义。
   public final static SLogicTableInfo TABLE = new SLogicTableInfo("logger.info.browser.access", "LG_INF_BROWSER_ACCESS");
   ...
   // 字段浏览器编号的定义。
   public final static SLogicFieldInfo BROWSER_ID = new SLogicFieldInfo("BROWSER_ID");

   // 字段访问页面的定义。
   public final static SLogicFieldInfo PAGE_URI = new SLogicFieldInfo("PAGE_URI");
   ...
   // 字段集合的定义。
   public final static String FIELDS = "`OUID`,`OVLD`,`GUID`,`BROWSER_ID`,`PAGE_URI`,`CREATE_USER_ID`,`CREATE_DATE`,`UPDATE_USER_ID`,`UPDATE_DATE`";

   //============================================================
   // <T>构造信息浏览器访问逻辑单元。</T>
   //============================================================
   public FLoggerInfoBrowserAccessLogic(){
      _name = TABLE.name();
      _classUnit = FLoggerInfoBrowserAccessUnit.class;
   }
   ...
   // @param context 逻辑环境
   //============================================================
   public FLoggerInfoBrowserAccessLogic(ILogicContext context){
      super(context);
      _name = TABLE.name();
      _classUnit = FLoggerInfoBrowserAccessUnit.class;
   }
   ...
   public <T extends FLogicUnit> T find(T unit, Class<T> clazz, long recordId){
      ...
      if(unit == null){
         if(clazz == null){
            unit = (T)(new FLoggerInfoBrowserAccessUnit());
         }else{
   ...
   // @return 数据单元
   //============================================================
   public FLoggerInfoBrowserAccessUnit findByGuid(CharSequence guid){
      return findByGuid(null, FLoggerInfoBrowserAccessUnit.class, guid);
   }
   ...
   // @return 数据单元
   //============================================================
   public FLoggerInfoBrowserAccessUnit search(CharSequence whereSql){
      return search(null, FLoggerInfoBrowserAccessUnit.class, whereSql);
   }
   ...
   public <T extends FLogicUnit> FLogicDataset<T> fetchSql(Class<T> clazz, CharSequence code, CharSequence sql, int pageSize, int page){
      ...
      if(clazz == null){
         result = (FLogicDataset<T>)(new FLogicDataset<FLoggerInfoBrowserAccessUnit>(FLoggerInfoBrowserAccessUnit.class, _logicContext));
      }else{
   ...
   // @return 数据单元集合
   //============================================================
   public FLogicDataset<FLoggerInfoBrowserAccessUnit> fetchAll(){
      // 生成命令
   ...
   // @return 数据单元
   //============================================================
   public FLoggerInfoBrowserAccessUnit doPrepare(){
      FLoggerInfoBrowserAccessUnit unit = new FLoggerInfoBrowserAccessUnit();
      unit.linkLogicContext(_logicContext);
      doPrepare(unit);
      return unit;
   }
   ...
   public EResult doInsert(FLogicUnit logicUnit){
      ...
      cmd.append('\'');
      cmd.append(',');
      long browserId = unit.browserId();
      if(browserId == 0){
         cmd.append("NULL");
      }else{
         cmd.append(browserId);
      }
      cmd.append(',');
      String pageUri = unit.pageUri();
      if(RString.isEmpty(pageUri)){
         cmd.append("NULL");
      }else{
         cmd.append('\'');
         cmd.append(RSql.formatValue(pageUri));
         cmd.append('\'');
      }
      // 设置更新信息
