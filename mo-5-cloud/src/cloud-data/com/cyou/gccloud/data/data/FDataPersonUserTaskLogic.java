...
//============================================================
@ASourceMachine
public class FDataPersonUserTaskLogic
      extends FLogicTable
{
   // 用户任务表的定义。
   ...
   // 用户任务表的定义。
   public final static SLogicTableInfo TABLE = new SLogicTableInfo("data.person.user.task", "DT_PSN_USER_TASK");
   ...
   // 字段任务编号的定义。
   public final static SLogicFieldInfo TASK_ID = new SLogicFieldInfo("TASK_ID");

   // 字段任务完成状态的定义。
   public final static SLogicFieldInfo TASK_STATUS_CD = new SLogicFieldInfo("TASK_STATUS_CD");

   // 字段任务完成次数的定义。
   public final static SLogicFieldInfo TASK_FINISH_COUNT = new SLogicFieldInfo("TASK_FINISH_COUNT");

   // 字段任务数据的定义。
   public final static SLogicFieldInfo TASK_DATA = new SLogicFieldInfo("TASK_DATA");
   ...
   // 字段集合的定义。
   public final static String FIELDS = "OUID,OVLD,GUID,USER_ID,TASK_ID,TASK_STATUS_CD,TASK_FINISH_COUNT,TASK_DATA,LINK_ID,SCORE_BEAN,SCORE_POINT,SCORE_GOLD,CREATE_USER_ID,CREATE_DATE,UPDATE_USER_ID,UPDATE_DATE";

   //============================================================
   // <T>构造用户任务表逻辑单元。</T>
   //============================================================
   public FDataPersonUserTaskLogic(){
      _name = TABLE.name();
      _classUnit = FDataPersonUserTaskUnit.class;
   }
   ...
   // @param context 逻辑环境
   //============================================================
   public FDataPersonUserTaskLogic(ILogicContext context){
      super(context);
      _name = TABLE.name();
      _classUnit = FDataPersonUserTaskUnit.class;
   }
   ...
   public <T extends FLogicUnit> T find(T unit,
      ...
      if(unit == null){
         if(clazz == null){
            unit = (T)(new FDataPersonUserTaskUnit());
         }else{
   ...
   // @return 数据单元
   //============================================================
   public FDataPersonUserTaskUnit search(CharSequence whereSql){
      return search(null, FDataPersonUserTaskUnit.class, whereSql);
   }
   ...
   // @return 数据单元集合
   //============================================================
   public FLogicDataset<FDataPersonUserTaskUnit> fetch(int pageSize,
                                                       int page){
   ...
   // @return 数据单元集合
   //============================================================
   public FLogicDataset<FDataPersonUserTaskUnit> fetch(CharSequence whereSql,
                                                       int pageSize,
                                                       int page){
   ...
   // @return 数据单元集合
   //============================================================
   public FLogicDataset<FDataPersonUserTaskUnit> fetch(CharSequence fields,
                                                       CharSequence whereSql,
                                                       CharSequence orderSql,
                                                       int pageSize,
                                                       int page){
   ...
   // @return 数据单元集合
   //============================================================
   public FLogicDataset<FDataPersonUserTaskUnit> fetchSql(CharSequence code,
                                                          CharSequence sql,
                                                          int pageSize,
                                                          int page){
   ...
   public <T extends FLogicUnit> FLogicDataset<T> fetchSql(Class<T> clazz,
      ...
      if(clazz == null){
         result = (FLogicDataset<T>)(new FLogicDataset<FDataPersonUserTaskUnit>(FDataPersonUserTaskUnit.class, _logicContext));
      }else{
   ...
   // @return 数据单元集合
   //============================================================
   public FLogicDataset<FDataPersonUserTaskUnit> fetchAll(){
      // 生成命令
   ...
   // @return 数据单元
   //============================================================
   public FDataPersonUserTaskUnit doPrepare(){
      FDataPersonUserTaskUnit unit = new FDataPersonUserTaskUnit();
      unit.linkLogicContext(_logicContext);
      doPrepare(unit);
      return unit;
   }
   ...
   public EResult doPrepare(FLogicUnit logicUnit){
      FDataPersonUserTaskUnit unit = (FDataPersonUserTaskUnit)logicUnit;
      unit.setOvld(true);
   ...
   public EResult doInsert(FLogicUnit logicUnit){
      ...
      cmd.append(",`USER_ID`");
      cmd.append(",`TASK_ID`");
      cmd.append(",`TASK_STATUS_CD`");
      cmd.append(",`TASK_FINISH_COUNT`");
      cmd.append(",`TASK_DATA`");
      cmd.append(",`LINK_ID`");
      ...
      cmd.append(',');
      cmd.append(unit.taskStatusCd());
      cmd.append(',');
      cmd.append(unit.taskFinishCount());
      cmd.append(',');
      String taskData = unit.taskData();
      if(RString.isEmpty(taskData)){
         cmd.append("NULL");
      }else{
         cmd.append('\'');
         cmd.append(RSql.formatValue(taskData));
         cmd.append('\'');
      }
      cmd.append(',');
   ...
   public EResult doUpdate(FLogicUnit logicUnit,
      ...
      if(unit.isTaskStatusCdChanged()){
         cmd.append(",`TASK_STATUS_CD`=");
         cmd.append(unit.taskStatusCd());
      }
      if(unit.isTaskFinishCountChanged()){
         cmd.append(",`TASK_FINISH_COUNT`=");
         cmd.append(unit.taskFinishCount());
      }
      if(unit.isTaskDataChanged()){
         cmd.append(",`TASK_DATA`=");
         String taskData = unit.taskData();
         if(RString.isEmpty(taskData)){
            cmd.append("NULL");
