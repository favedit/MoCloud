...
public class FDataPersonUserTruetimedataLogic extends FLogicTable
{
   ...
   // 人员实时数据访问的定义。
   public final static SLogicTableInfo TABLE = new SLogicTableInfo("data.person.user.truetimedata", "DT_PSN_USER_TRUETIME");
   ...
   // 字段实时数据编号的定义。
   public final static SLogicFieldInfo TRUETIME_ID = new SLogicFieldInfo("TRUETIME_ID");
   ...
   // 字段集合的定义。
   public final static String FIELDS = "`OUID`,`OVLD`,`GUID`,`USER_ID`,`TRUETIME_ID`,`ACTIVE_CD`,`ACTIVE_DATE`,`NOTE`,`CREATE_USER_ID`,`CREATE_DATE`,`UPDATE_USER_ID`,`UPDATE_DATE`";

   //============================================================
   // <T>构造人员实时数据访问逻辑单元。</T>
   //============================================================
   public FDataPersonUserTruetimedataLogic(){
      _name = TABLE.name();
      _classUnit = FDataPersonUserTruetimedataUnit.class;
   }
   ...
   // @param context 逻辑环境
   //============================================================
   public FDataPersonUserTruetimedataLogic(ILogicContext context){
      super(context);
      _name = TABLE.name();
      _classUnit = FDataPersonUserTruetimedataUnit.class;
   }
   ...
   public <T extends FLogicUnit> T find(T unit, Class<T> clazz, long recordId){
      ...
      if(unit == null){
         if(clazz == null){
            unit = (T)(new FDataPersonUserTruetimedataUnit());
         }else{
   ...
   // @return 数据单元
   //============================================================
   public FDataPersonUserTruetimedataUnit findByGuid(CharSequence guid){
      return findByGuid(null, FDataPersonUserTruetimedataUnit.class, guid);
   }
   ...
   // @return 数据单元
   //============================================================
   public FDataPersonUserTruetimedataUnit search(CharSequence whereSql){
      return search(null, FDataPersonUserTruetimedataUnit.class, whereSql);
   }
   ...
   public <T extends FLogicUnit> FLogicDataset<T> fetchSql(Class<T> clazz, CharSequence code, CharSequence sql, int pageSize, int page){
      ...
      if(clazz == null){
         result = (FLogicDataset<T>)(new FLogicDataset<FDataPersonUserTruetimedataUnit>(FDataPersonUserTruetimedataUnit.class, _logicContext));
      }else{
   ...
   // @return 数据单元集合
   //============================================================
   public FLogicDataset<FDataPersonUserTruetimedataUnit> fetchAll(){
      // 生成命令
   ...
   // @return 数据单元
   //============================================================
   public FDataPersonUserTruetimedataUnit doPrepare(){
      FDataPersonUserTruetimedataUnit unit = new FDataPersonUserTruetimedataUnit();
      unit.linkLogicContext(_logicContext);
      doPrepare(unit);
      return unit;
   }
   ...
   public EResult doInsert(FLogicUnit logicUnit){
      ...
      cmd.append(',');
      long truetimeId = unit.truetimeId();
      if(truetimeId == 0){
         cmd.append("NULL");
      }else{
         cmd.append(truetimeId);
      }
      cmd.append(',');
