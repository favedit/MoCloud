...
// <T>用户控制台。</T>
//============================================================
public class FGcSessionConsole extends
        FAbstractLogicUnitConsole<FCacheSystemSessionLogic, FGcSessionInfo>
        implements IGcSessionConsole {
    // 日志输出接口
    ...
    public FGcSessionInfo findBySessionCode(ILogicContext context,
            String logicCode, String fromCode, String sessionCode) {
        // 生成SQL文
        FSql whereSql = new FSql();
        whereSql.append("(" + FCacheSystemSessionLogic.LOGIC_CODE
                + "={logic_code})");
        whereSql.append(" AND (" + FCacheSystemSessionLogic.FROM_CODE
                + "={from_code})");
        whereSql.append(" AND (" + FCacheSystemSessionLogic.SESSION_CODE
                + "={session_code})");
        ...
        FGcSessionInfo sessionInfo = logic.search(FGcSessionInfo.class,
                whereSql);
        return sessionInfo;
    }
    ...
    public FLogicDataset<FGcSessionInfo> findByUserId(ILogicContext context,
        ...
        String whereSql = "(" + FCacheSystemSessionLogic.USER_ID + "=" + userId
                + ") AND (" + FCacheSystemSessionLogic.OVLD + "=1)";
        FCacheSystemSessionLogic logic = findLogic(context);
        FLogicDataset<FGcSessionInfo> sessions = logic.fetchClass(
                FGcSessionInfo.class, whereSql);
        return sessions;
    }
    ...
    public FLogicDataset<FGcSessionInfo> findByUserAndAppId(
        ...
        String whereSql = "(" + FCacheSystemSessionLogic.USER_ID + "=" + userId
                + ") AND (" + FCacheSystemSessionLogic.OVLD + "=1) AND ("
                + FCacheSystemSessionLogic.APPLICATION_ID + "=" + applicationId;
        FCacheSystemSessionLogic logic = findLogic(context);
        FLogicDataset<FGcSessionInfo> sessions = logic.fetchClass(
                FGcSessionInfo.class, whereSql);
        return sessions;
    ...
    // FCacheSystemSessionLogic logic = findLogic(context);
    // // 新建记录
    // FGcSessionInfo session = logic.findByGuid(FGcSessionInfo.class,
    // sessionId);
    // if(session == null){
    ...
    // logic.doInsert(session);
    // _logger.debug(this, "open",
    // "Open session. (guid={1}, user_id={2}, from_cd={3})", session.guid(),
    // userId, fromCd);
    // // 绑定数据
    ...
    // "') AND (OVLD=1)";
    // FCacheSystemSessionLogic sessionLogic =
    // context.findLogic(FCacheSystemSessionLogic.class);
    // FGcSessionInfo sessionInfo = sessionLogic.search(FGcSessionInfo.class,
    // whereSql);
    // // 检查会话信息
    // if(sessionInfo == null){
    // _logicMessageConsole.throwError(messageContext, "E00103", code);
    ...
    // String whereSql = FCacheSystemSessionLogic.USER_ID + "=" + userId;
    // FCacheSystemSessionLogic logic = findLogic(context);
    // FLogicDataset<FCacheSystemSessionUnit> sessions = logic.fetch(whereSql,
    // 100, 0);
    // // 删除当前用户所有会话
    // if(sessions != null){
